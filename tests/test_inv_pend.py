...
#     loop_model.train(train_data=data_train, epochs=20, batch_size=128, lr=1e-3)
...
#     import tensorflow as tf
#     predictions = loop_model({k: tf.expand_dims(tf.expand_dims(tf.convert_to_tensor(v), axis=0), axis=-1) for k, v in data_train[0].items()})
