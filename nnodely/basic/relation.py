...
from nnodely.basic.modeldef import ModelGraph, get_current_model_graph, set_current_model_graph
from nnodely.nnodely import get_manager, set_manager
...
class Relation(Stream):
    def __init__(self, name_node, edges, **attrs):
        super().__init__(name_node, **attrs)
    ...
    def __set_edges__(self, edges:list|str):
        ...
        for edge in edges:
            edge_attrs = {}
            mg.set_edge(edge, self.name, **edge_attrs)

    def __build__(self, name:str, from_node:str):
        return super().__build__(name, from_node)
    ...
    def connect(self, obj) -> "Stream":
        ...
        """
        mg = get_current_model_graph()
        self.attrs['connect'] = obj.name
        mg.set_node_attr(node_name=obj.name, connect=self.name)
        mg.set_edge(from_node=self.name, to_node=obj.name, type='connect')


    def loop(self, obj) -> "Stream":
        """
        Update the Stream adding a closed loop connection with a given input object.
        ...
        """
        mg = get_current_model_graph()
        mg.set_node_attr(node_name=obj.name, loop=self.name)
        mg.set_edge(from_node=self.name, to_node=obj.name, type='loop')
