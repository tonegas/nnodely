...
In particular, we extract a time window :math:`T_w` of 0.25 second.
...
window :math:`T_w = 0.25` second, which means that we are using the last :math:`N_x` samples of the
...
.. code-block:: python
 ...
 mass_spring_damper.addMinimize('next-pos', Input('x_r').z(-1), x_z_est, 'mse')
...
the available data. In this example, :math:`T_s = 0.05` seconds. The training dataset is then
...
Note that the input variable x is a list of 5 samples plus one sample, as the sampling time :math:`T_s` is 0.05 seconds and the time window :math:`T_w` of the input variable x is 0.25 second.
...
For the tutorial please refer to the link below.
