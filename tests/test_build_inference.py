...
def test_basic_model_build_and_inference():
    x = Input("x", dim=1)
    y = Input("y", dim=1)
    ...
    dummy_x = np.ones((4, 1, 10), dtype=np.float32)
    dummy_y = np.ones((4, 1, 10), dtype=np.float32)

    result = model([dummy_x, dummy_y])

    assert "x_pred" in result
    assert result["x_pred"].shape == (4, 2, 1)


def test_model_inference_with_composed_model():
    ...
    assert result1["x_pred"].shape == (4, 2, 1)

    result2 = model2({"z": dummy_input_z})
    assert "z_pred" in result2
    assert result2["z_pred"].shape == (4, 1, 1)
