...
def test_layer_reuse_across_ode_stages():
    # RK4 applies the field four times, which is where reuse used to collide.
    y = Input("stage_y", dim=1)
    field = Linear(out_features=1, use_bias=False, initializer="ones")
    step = Ode(field, y.last(), 0.1, method="rk4")
    model = Modely("reuse_ode", inputs=[y], outputs=[Output("stage_out", step)]).build()
    assert model.model is not None
    kernels = [
        variable
        for variable in model.model.trainable_variables
    ...
    assert len(kernels) == 1
    kernels[0].assign(np.full((1, 1), -1.0, dtype=np.float32))

    result = model({"stage_y": np.ones((1, 1, 1), dtype=np.float32)})
    # One RK4 step of dy/dt = -y from y = 1 approximates exp(-0.1).
    assert float(to_numpy(result["stage_out"]).ravel()[0]) == pytest.approx(
        np.exp(-0.1), abs=1e-6
    )


def test_layer_reuse_supports_neural_field():
    # The point of the fix: a genuine MLP vector field under Ode, whose hidden
    # layers are evaluated once per stage and must share weights.
    y = Input("mlp_y", dim=2)
    hidden = Linear(out_features=8, use_bias=True)
    readout = Linear(out_features=2, use_bias=False)
    step = Ode(lambda state: readout(Tanh()(hidden(state))), y.last(), 0.05)
    model = Modely("reuse_mlp", inputs=[y], outputs=[Output("mlp_out", step)]).build()
    assert model.model is not None

    # Two Linear kernels plus one bias, each appearing exactly once however many
    # stages evaluate them.
    assert len(model.model.trainable_variables) == 3
    result = to_numpy(model({"mlp_y": np.ones((4, 2, 1), dtype=np.float32)})["mlp_out"])
    assert result.shape == (4, 2, 1)
    assert np.all(np.isfinite(result))


def test_layer_reuse_neural_field_is_trainable():
    # Sharing is only useful if the gradient reaches the shared weights through
    # every stage, so train the field briefly and check it actually moves.
    y = Input("trainable_y", dim=2)
    hidden = Linear(out_features=8, use_bias=True)
    readout = Linear(out_features=2, use_bias=False)
    step = Ode(lambda state: readout(Tanh()(hidden(state))), y.last(), 0.1)
    predicted = Output("trainable_pred", step)

    model = Modely("reuse_trainable", inputs=[y], outputs=[predicted])
    model.minimize(
        "err", source=predicted, target=Input("trainable_target", dim=2), loss="mse"
    )
    model.build()
    assert model.model is not None
    assert len(model.model.trainable_variables) == 3
    before = [to_numpy(variable).copy() for variable in model.model.trainable_variables]

    samples = np.linspace(-1.0, 1.0, 64, dtype=np.float32)
    states = np.stack([samples, samples[::-1]], axis=-1)
    data = DataLoader(
        model, source={"trainable_y": states, "trainable_target": 0.5 * states}
    )
    history = model.train(
        train_data=data, epochs=30, batch_size=16, lr=0.01, optimizer="adam"
    )

    assert history["loss"][-1] < history["loss"][0]
    after = [to_numpy(variable) for variable in model.model.trainable_variables]
