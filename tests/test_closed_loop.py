...
def test_closed_loop(tmp_path):
    ...
    result_in = model_in({"_x": dummy_input_x, "_y": dummy_input_y})
    assert "out1" in result_in
    assert result_in["out1"].shape == (batch_size, 1, 1, 4)
...
def test_sw_closed_loop():
    ...
    z = Input(name="z", dim=1, seq=(4, 2))
    loop_fn2 = Loop(f=model_in, closed_loop={"z": "out1"}, name="loop_model_in")
    out_w = Output("out_w", loop_fn2([w.sw(2), z]))
    model_out = Modely(name="model_with_loop_w", inputs=[w, z], outputs=[out_w])
...
def test_sw_2_closed_loop():
    ...
    model_out.minimize(
        "error",
        source=out_w,
        target=Input("w_target", dim=1, seq=(4, 2)).sw(2),
        loss="mse",
    )
