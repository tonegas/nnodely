...
def test_inv_pend_loop(tmp_path):
    ...
    # l = Parameter(name="l", dim=1, value=0.3)       # half the length of the pendulum
    ...
    for key_t, key_p in zip(["Ypos_s", "Yvelocity_s", "Yangle_s", "Yangular_velocity_s"], ["Ypos_pred_s", "Yvel_pred_s", "Yang_pred_s", "Yang_vel_s"]):
        ...
        plt.ylabel(key_t)
        plt.legend()
        plt.savefig(os.path.join(tmp_path, f"{key_t}_prediction.png"))
        plt.close()

def test_inv_pend_roll(tmp_path):
    # Define inputs
    pos = Input(name="Xpos", dim=1)
    vel = Input(name="Xvelocity", dim=1)
    angle = Input(name="Xangle", dim=1)
    ...
    steps = 1
    model.rollback(steps=steps, rollback={
        "Xpos": "Ypos_pred",
        "Xvelocity": "Yvelocity_pred",
        "Xangle": "Yangle_pred",
        "Xangular_velocity": "Yangular_velocity_pred"
    ...
    loop_model.minimize("error_pos", source=out_pos, target=Input(name="Ypos_s", dim=1).next(), loss="mse")
    loop_model.minimize("error_vel", source=out_vel, target=Input(name="Yvelocity_s", dim=1).next(), loss="mse")
    loop_model.minimize("error_angle", source=out_angle, target=Input(name="Yangle_s", dim=1).next(), loss="mse")
    loop_model.minimize("error_ang_vel", source=out_ang_vel, target=Input(name="Yangular_velocity_s", dim=1).next(), loss="mse")
    loop_model.build()
    #loop_model.plot(to_file=os.path.join(tmp_path, "model_inv_pend.png"))
    loop_model.export_html(os.path.join("html", "model_inv_pend.html"))
    ...
    print("Target:", {k: v for k, v in test_data.items() if k in ["Ypos_s", "Yvelocity_s", "Yangle_s", "Yangular_velocity_s"]})
    reloaded_predictions = loaded_model(test_data)
    for key in [key for key in predictions.keys() if "Y" in key]:
        assert np.allclose(
            np.asarray(predictions[key]), np.asarray(reloaded_predictions[key]), atol=1e-5
        ), f"reloaded model diverged on {key}"
    #print("Outputs:", predictions)
    ...
    # make a plot of the predictions vs the target for each output
    import matplotlib.pyplot as plt
    for key_p, key_t in zip([key for key in predictions.keys() if "Y" in key], ["Ypos_s", "Yvelocity_s", "Yangle_s", "Yangular_velocity_s"]):
        plt.figure()
        plt.plot(24, predictions[key_p][0, 0, :], label="Prediction")
        plt.plot(test_data[key_t][0, 0, :], label="Target", linestyle=":")
        plt.title(key_t)
        plt.xlabel("Time step")
        plt.ylabel(key_t)
        plt.legend()
...
if __name__ == "__main__":
    #test_inv_pend(tmp_path="html")
    #test_inv_pend_loop(tmp_path="html")
    test_inv_pend_roll(tmp_path="html")